    ...
    QMainWindow(parent)
{
    ...
    QObject::connect(m_exit, SIGNAL(pressed()), this, SLOT(close()));
...
void CameraExample::searchAndLock()
{
    ...
    if (pictureCaptured) {
        ...
        if (m_camera->supportedLocks() & QCamera::LockFocus) {  //// MY PHONE NOT SUPPORTED FOCUS
            m_focusing = true;                                  //// UNCOMMENT THIS
...
void CameraExample::paintEvent(QPaintEvent *event)
{
    ...
    if (showViewFinder && m_myVideoSurface && m_myVideoSurface->isActive()) {
        ...
        if (pictureCaptured) {
            ...
            /////////////////////////////////////////////////////QDesktopServices::openUrl(QUrl("file://"+m_imageName));
        }
    }
